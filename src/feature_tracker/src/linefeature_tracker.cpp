...
{
    RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "reading paramerter of camera %s", calib_file.c_str());
...
void LineFeatureTracker::readImage(const cv::Mat &_img)
{
    ...
    //RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "undistortImage costs: %fms", t_p.toc());
    if (EQUALIZE)   // 直方图均衡化
    {
    ...
    //RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "ld inition costs: %fms", t_li.toc());
    TicToc t_ld;
    ...
    RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "line detect costs: %fms", t_ld.toc());
    ...
    if(curframe_->vecLine.size() > 0)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "line match costs: %fms", t_nlt.toc());
...
//    cv::waitKey(1);
    ...
    opts.refine       = 1.0;     //1     	The way found lines will be refined
    ...
    opts.log_eps      = 1.5;	//0		Detection threshold: -log10(NFA) > log_eps. Used only when advance refinement is chosen
    ...
    double min_line_length = 0.5;  // Line segments shorter than that are rejected
    // opts.refine       = 1;
    // opts.scale        = 0.5;
    ...
    sum_time += t_li.toc();
   RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "line detect costs: %fms", t_li.toc());
...
//////////////////////////
    ...
    // std::cout<<"lbd_descr = "<<lbd_descr.size()<<std::endl;
//    RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "lbd_descr detect costs: %fms", keylsd.size() * t_lbd.toc() / lsd.size() );
...
///////////////
    ...
    if(curframe_->keylsd.size() > 0)
    {
        ...
        bdm_ = BinaryDescriptorMatcher::createBinaryDescriptorMatcher();
        bdm_->match(forwframe_->lbd_descr, curframe_->lbd_descr, lsd_matches);
//        RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "lbd_macht costs: %fms", t_match.toc());
        sum_time += t_match.toc();
        mean_time = sum_time/frame_cnt;
        // RCLCPP_INFO(rclcpp::get_logger("linefeature_tracker"), "line feature tracker mean costs: %fms", mean_time);
        /* select best matches */
        std::vector<DMatch> good_matches;
