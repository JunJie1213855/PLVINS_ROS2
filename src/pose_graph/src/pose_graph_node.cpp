...
queue<plvins_interfaces::msg::PlvinsCloud::ConstSharedPtr> point_buf;
...
rclcpp::Publisher<plvins_interfaces::msg::PlvinsCloud>::SharedPtr pub_match_points;
...
void new_sequence()
{
    ...
    if (sequence > 5)
    {
        RCLCPP_WARN(rclcpp::get_logger("pose_graph"), "only support 5 sequences since it's boring to copy code for more sequences.");
        rclcpp::shutdown();
    }
...
{
    //RCLCPP_INFO(rclcpp::get_logger("pose_graph"), "image_callback!");
    if(!LOOP_CLOSURE)
        return;
    m_buf.lock();
    ...
    {
        RCLCPP_WARN(rclcpp::get_logger("pose_graph"), "image discontinue! detect a new sequence!");
        new_sequence();
    }
    last_image_time = rclcpp::Time(image_msg->header.stamp).seconds();
}

void point_callback(const plvins_interfaces::msg::PlvinsCloud::ConstSharedPtr &point_msg)
{
    //RCLCPP_INFO(rclcpp::get_logger("pose_graph"), "point_callback!");
    if(!LOOP_CLOSURE)
        return;
    m_buf.lock();
...
{
    //RCLCPP_INFO(rclcpp::get_logger("pose_graph"), "pose_callback!");
    if(!LOOP_CLOSURE)
        return;
    m_buf.lock();
...
{
    //RCLCPP_INFO(rclcpp::get_logger("pose_graph"), "vio_callback!");
...
void process()
{
    ...
    while (true)
    {
        ...
        plvins_interfaces::msg::PlvinsCloud::ConstSharedPtr point_msg = NULL;
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<rclcpp::Node>("pose_graph");
    posegraph.registerPub(node.get());
    ...
    auto sub_imu_forward = node->create_subscription<nav_msgs::msg::Odometry>("/plvins_estimator/imu_propagate", 2000, imu_forward_callback);
    auto sub_vio = node->create_subscription<nav_msgs::msg::Odometry>("/plvins_estimator/odometry", 2000, vio_callback);
    auto sub_image = node->create_subscription<sensor_msgs::msg::Image>(IMAGE_TOPIC, 2000, image_callback);
    auto sub_pose = node->create_subscription<nav_msgs::msg::Odometry>("/plvins_estimator/keyframe_pose", 2000, pose_callback);
    auto sub_extrinsic = node->create_subscription<nav_msgs::msg::Odometry>("/plvins_estimator/extrinsic", 2000, extrinsic_callback);
    auto sub_point = node->create_subscription<plvins_interfaces::msg::PlvinsCloud>("/plvins_estimator/keyframe_point", 2000, point_callback);
    auto sub_relo_relative_pose = node->create_subscription<nav_msgs::msg::Odometry>("/plvins_estimator/relo_relative_pose", 2000, relo_relative_pose_callback);
    ...
    pub_key_odometrys = node->create_publisher<visualization_msgs::msg::Marker>("key_odometrys", 1000);
    pub_vio_path = node->create_publisher<nav_msgs::msg::Path>("no_loop_path", 1000);
    pub_match_points = node->create_publisher<plvins_interfaces::msg::PlvinsCloud>("match_points", 100);
