...
{
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "init begins");
    clearState();
    failure_occur = 0;
}
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 5, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::msg::Header &header)
{
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "new image coming ------------------------------------------");
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "Adding feature points %lu", image.size());
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "this frame is--------------------%s", marginalization_flag ? "reject" : "accept");
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "%s", marginalization_flag ? "Non-keyframe" : "Keyframe");
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "Solving %d", frame_count);
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "number of feature: %d", f_manager.getFeatureCount());
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), "calibrating extrinsic param, rotation movement is needed");
        if (frame_count != 0)
        {
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("vins_est"), "initial extrinsic rotation calib success");
                RCLCPP_WARN_STREAM(rclcpp::get_logger("vins_est"), "initial extrinsic rotation: " << endl << calib_ric);
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if(result)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("vins_est"), "Initialization finish!");
    ...
    else
    {
        TicToc t_solve;
        solveOdometry();      // 三角化新特征 并 swf优化
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "solver costs: %fms", t_solve.toc());

        if (failureDetection())
        {
            RCLCPP_WARN(rclcpp::get_logger("vins_est"), "failure detection!");
            failure_occur = 1;
            clearState();
            setParameter();
            RCLCPP_WARN(rclcpp::get_logger("vins_est"), "system reboot!");
            return;
        }
        ...
        slideWindow();
        f_manager.removeFailures();
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "marginalization costs: %fms", t_margin.toc());
...
// void Estimator::processImage(const map<int, vector<pair<int, Vector3d>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::msg::Header &header)
// // void Estimator::processImage(const map<int, vector<pair<int, Vector3d>>> &image,const map<int, vector<pair<int, Vector3d>>> &image1, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::msg::Header &header)
// {
//     RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "new image coming ------------------------------------------");
//     RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "Adding feature points %lu", image.size());
//     //if (f_manager.addFeatureCheckParallax(frame_count, image))           // 当视差较大时，marg 老的关键帧
...
void Estimator::processImage(const map<int, vector<pair<int, Vector3d>>> &image, const map<int, vector<pair<int, Vector8d>>> &lines, const std_msgs::msg::Header &header)
{
...
void Estimator::processImage(const map<int, vector<pair<int, Vector3d>>> &image, const std_msgs::msg::Header &header)
{
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "new image coming ------------------------------------------");
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "Adding feature points %lu", image.size());
    if (f_manager.addFeatureCheckParallax(frame_count, image))           // 当视差较大时，marg 老的关键帧
...
bool Estimator::initialStructure()
{
    ...
    //check imu observibility
    {
        ...
        //RCLCPP_WARN(rclcpp::get_logger("vins_est"), "IMU variation %f!", var);
        if(var < 0.25)
        {
            RCLCPP_INFO(rclcpp::get_logger("vins_est"), "IMU excitation not enouth!");
            //return false;
        }
    }
    // global sfm
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), "Not enough features or parallax; Move device around");
        return false;
    }
    GlobalSFM sfm;
    ...
    {
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "global SFM failed!");
        marginalization_flag = MARGIN_OLD;
        return false;
    }
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if(pts_3_vector.size() < 6)
        {
            ...
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "Not enough points for solve pnp !");
            return false;
        }
        if (! cv::solvePnP(pts_3_vector, pts_2_vector, K, D, rvec, t, 1))
        {
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "solve pnp fail!");
            return false;
        }
        cv::Rodrigues(rvec, r);
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), "misalign visual structure with IMU");
        return false;
    }
...
bool Estimator::visualInitialAlign()
{
    ...
    if(!result)
    {
        RCLCPP_WARN(rclcpp::get_logger("vins_est"), "solve g failed!");
        return false;
    }
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("vins_est"), "g0     " << g.transpose());
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("vins_est"), "my R0  " << Utility::R2ypr(Rs[0]).transpose()); 
...
//            if(average_parallax * 460 > 15 && m_estimator.solveRelativeRT(corres, relative_R, relative_T)) // 30
            ...
            {
                l = i;
                RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "average_parallax %f choose l %d and newest frame to triangulate the whole structure", average_parallax * 460, l);
                return true;
            }
        }
    }
...
void Estimator::solveOdometry()
{
    ...
    if (solver_flag == NON_LINEAR)
    {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "triangulation costs %f", t_tri.toc());
...
bool Estimator::failureDetection()
{
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), " little feature %d", f_manager.last_track_num);
        return true;
    }
    if (Bas[WINDOW_SIZE].norm() > 2.5)
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), " big IMU acc bias estimation %f", Bas[WINDOW_SIZE].norm());
        return true;
    }
    if (Bgs[WINDOW_SIZE].norm() > 1.0)
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), " big IMU gyr bias estimation %f", Bgs[WINDOW_SIZE].norm());
        return true;
    }
    /*
    if (tic(0) > 1)
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), " big extri param estimation %d", tic(0) > 1);
        return true;
    }
    */
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), " big translation");
        return true;
    }
    if (abs(tmp_P.z() - last_P.z()) > 1)
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), " big z translation");
        return true; 
    }
    ...
    if (delta_angle > 50)
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_est"), " big delta_angle ");
        return true;
    }
    return false;
}
...
void Estimator::optimizationwithLine()
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)         // 外参数
    {
        ...
        {
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "fix extinsic param");
            problem.SetParameterBlockConstant(para_Ex_Pose[i]);
        }
        else
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "estimate extinsic param");
    }
...
/////////////////////////////////////
    ...
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "lineFactor: %d, pointFactor:%d", line_m_cnt, f_m_cnt);
...
////////////////////////////////////////
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "visual measurement count: %d", f_m_cnt);
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "prepare for ceres: %f", t_prepare.toc());
...
//    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "Points Lines Iterations : %d", static_cast<int>(summary.iterations.size()));
    sum_solver_time_ += t_solver.toc();
    mean_solver_time_ = sum_solver_time_/frame_cnt_;
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "Points Lines solver costs: %f", mean_solver_time_);
    ...
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "culling line feautre: %f ms", t_culling.toc());
...
//                        drop_set = vector<int>{0, 2};   // marg pose and feature,  !!!! do not need marg, just drop they  !!!
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "pre marginalization %f ms", t_pre_margin.toc());
        
        TicToc t_margin;
        marginalization_info->marginalize();
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "marginalization %f ms", t_margin.toc());
    ...
    else
    {
            ...
            std::count(std::begin(last_marginalization_parameter_blocks), std::end(last_marginalization_parameter_blocks), para_Pose[WINDOW_SIZE - 1]))
        {
            ...
            TicToc t_pre_margin;
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "begin marginalization");
            marginalization_info->preMarginalize();
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "end pre marginalization, %f ms", t_pre_margin.toc());

            TicToc t_margin;
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "begin marginalization");
            marginalization_info->marginalize();
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "end marginalization, %f ms", t_margin.toc());
    ...
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "whole marginalization costs: %f", mean_marg_time_);

    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "whole time for ceres: %f", t_whole.toc());
}
...
//        options.max_solver_time_in_seconds = SOLVER_TIME;
    ...
    sum_solver_time_ += t_solver.toc();
    mean_solver_time_ = sum_solver_time_/frame_cnt_;
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "whole solver costs: %f", mean_solver_time_);
    //cout << summary.BriefReport() << endl;
//    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "Only Points Iterations : %d", static_cast<int>(summary.iterations.size()));
//    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "Only Points solver costs: %f", t_solver.toc());
    ...
    mean_marg_time_ = sum_marg_time_/frame_cnt_;
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "whole marginalization costs: %f", mean_marg_time_);
//    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "whole marginalization costs: %f", t_whole_marginalization.toc());
