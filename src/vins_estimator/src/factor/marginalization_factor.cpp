...
{
    //RCLCPP_WARN(rclcpp::get_logger("vins_est"), "release marginlizationinfo");
...
void MarginalizationInfo::marginalize()
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "marginalization, pos: %d, m: %d, n: %d, size: %d", pos, m, n, (int)parameter_block_idx.size());
    ...
    RCLCPP_INFO(rclcpp::get_logger("vins_est"), "summing up costs %f ms", t_summing.toc());
    */
    //multi thread
    ...
    for (int i = 0; i < NUM_THREADS; i++)
    {
        ...
        if (ret != 0)
        {
            RCLCPP_WARN(rclcpp::get_logger("vins_est"), "pthread_create error");
            rclcpp::shutdown();
        }
    }
    ...
    //RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "thread summing up costs %f ms", t_thread_summing.toc());
    //RCLCPP_INFO(rclcpp::get_logger("vins_est"), "A diff %f , b diff %f ", (A - tmp_A).sum(), (b - tmp_b).sum());
    ...
    if(m1 > 0 )
    {
        ...
        // RCLCPP_INFO(rclcpp::get_logger("vins_est"), "invese Amm costs %f ms", tmarg_l.toc());
    ...
    A = Arr - tempB * Amr;
    b = brr - tempB * bmm;
    // RCLCPP_INFO(rclcpp::get_logger("vins_est"), "marg pose costs %f ms", tmarg_pose.toc());
    /////////////////////////////////////////////////////////////
