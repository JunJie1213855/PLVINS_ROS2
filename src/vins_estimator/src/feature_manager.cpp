...
{
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "input feature: %d", (int)image.size());
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "num of feature: %d", getFeatureCount());  // 已有的特征数目
    ...
    {
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "parallax_sum: %lf, parallax_num: %d", parallax_sum, parallax_num);
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "current parallax: %lf", parallax_sum / parallax_num * FOCAL_LENGTH);
...
{
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "input point feature: %d", (int)image.size());
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "input line feature: %d", (int)lines.size());
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "num of feature: %d", getFeatureCount());  // 已有的特征数目
    double parallax_sum = 0;
    int parallax_num = 0;
    last_track_num = 0;
...
{
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "debug show");
    for (auto &it : feature)
    {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "%d,%d,%d ", it.feature_id, it.used_num, it.start_frame);
        int sum = 0;
        for (auto &j : it.feature_per_frame)
        {
            RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "%d,", int(j.is_used));
            sum += j.is_used;
...
void FeatureManager::setDepth(const VectorXd &x)
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        //RCLCPP_INFO(rclcpp::get_logger("vins_est"), "feature id %d , start_frame %d, depth %f ", it_per_id->feature_id, it_per_id-> start_frame, it_per_id->estimated_depth);
