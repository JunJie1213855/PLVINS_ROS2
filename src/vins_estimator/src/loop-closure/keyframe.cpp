...
void KeyFrame::searchByDes(std::vector<cv::Point2f> &measurements_old, 
                           ...
                           const camodocal::CameraPtr &m_camera)
{
    //RCLCPP_INFO(rclcpp::get_logger("vins_est"), "loop_match before cur %d %d, old %d", (int)window_descriptors.size(), (int)measurements.size(), (int)descriptors_old.size());
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "t_pnp_ransac %f ms", t_pnp_ransac.toc());
                                          ...
                                          const camodocal::CameraPtr &m_camera)
{
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "loop final use num %d %lf---------------", (int)measurements_old.size(), t_match.toc());
    return true;
}
