...
{
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "add keyframe begin!");
...
*/
	...
	odometry.header.stamp = rclcpp::Time(pKF->header);
	...
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "add keyframe end!");

}

void KeyFrameDatabase::downsample(vector<int> &erase_index)
{
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "resample keyframe begin!");
	unique_lock<mutex> lock(mMutexkeyFrameList);
	...
	lock.unlock();
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "resample keyframe end!");
}
...
{
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "optimizae pose graph begin!");
	...
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "optimizae pose graph end!");

}

void KeyFrameDatabase::updateVisualization()
{
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "updateVisualization begin");
	unique_lock<mutex> mlockPath(mPath);
	...
	for (it = keyFrameList.begin(); it != keyFrameList.end(); it++)
	{
		...
		*/
		...
		// add key frame to path for visualization
		nav_msgs::msg::Odometry odometry;
		odometry.header.stamp = rclcpp::Time((*it)->header);
		odometry.header.frame_id = "world";
		odometry.pose.pose.position.x = P.x();
		odometry.pose.pose.position.y = P.y();
		odometry.pose.pose.position.z = P.z();
	...
	RCLCPP_DEBUG(rclcpp::get_logger("vins_est"), "updateVisualization end");
}
...
nav_msgs::msg::Path KeyFrameDatabase::getPath()
{
